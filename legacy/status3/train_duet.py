...
SCENARIO_DIR = PROJECT_ROOT / "status3"
...
from duet_repro.engines.ultralytics_patch import DuETDetectionTrainer
...
def project_relative_value(value):
    """Convert project-local paths in metadata to portable relative strings."""
    if isinstance(value, dict):
        return {key: project_relative_value(item) for key, item in value.items()}
    if isinstance(value, list):
        return [project_relative_value(item) for item in value]
...
def write_experiment_state(
    ...
    *,
    ...
    portable_history = project_relative_value(history)
    portable_cfg = project_relative_value(cfg)
    ...
    manifest = {
        "reference_checkpoint": project_relative_value(reference_ckpt),
        "latest_checkpoint": project_relative_value(latest_ckpt) if latest_ckpt is not None else None,
        "output_dir": project_relative_value(output_dir),
...
def main(
    config_path: str | Path = SCENARIO_DIR / "configs" / "train.yaml",
    *,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default=str(SCENARIO_DIR / "configs" / "train.yaml"))
