...
def _is_detect_head_key(key: str) -> bool:
    """Return whether a state_dict key belongs to YOLO11 Detect head model.23."""
    return key.startswith("model.23.")


def _clone_head_state(state: StateDict) -> StateDict:
    """Clone a complete Detect head state_dict slice onto CPU for artifact saving."""
    return {
        key: value.detach().cpu().clone()
        for key, value in state.items()
        if _is_detect_head_key(key)
    }


def save_parallel_detect_heads_checkpoint(
    compatible_checkpoint: str | Path,
    ...
    *,
    ...
    """
    Save a checkpoint artifact that keeps two complete Detect heads in parallel.

    The normal ``model`` inside the checkpoint remains YOLO-compatible. The two
    complete task-specific heads are stored under ``duet_parallel_heads``:

      - ``old_detect_head``: previous task/T1 Detect head
      - ``new_detect_head``: current task/T2 trained Detect head

    This is a real two-head artifact for later custom inference/evaluation, but
    it does not pretend that stock Ultralytics can execute two heads without a
    custom forward path.
    """
    compatible_checkpoint = Path(compatible_checkpoint)
    output_path = Path(output_path)
    ckpt = torch_load_checkpoint(compatible_checkpoint, map_location="cpu")
    ckpt["duet_parallel_heads"] = {
        "mode": "parallel_complete_detect_heads",
        "detect_head_prefix": "model.23",
        "old_class_indices": [int(index) for index in learned_indices],
        "new_class_indices": [int(index) for index in current_indices],
        "old_detect_head": _clone_head_state(old_state),
        "new_detect_head": _clone_head_state(new_state),
        "note": (
            "Stock YOLO inference still uses the compatible single Detect head in model. "
            "Use duet_parallel_heads for a custom two-head forward/evaluation path."
        ),
    }
    output_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(ckpt, output_path)
    print(f"[Parallel Head] saved two complete Detect heads to: {output_path}")
    return output_path
    ...
    *,
    ...
    for task_index, task in enumerate(cfg["tasks"], start=1):
        ...
        if task_index == 1 or not duet_cfg.get("enabled", True):
            ...
            inject_state_dict_into_checkpoint(trained_ckpt, merged_state, merged_ckpt)
            parallel_head_ckpt = output_dir / f"task_{task_index}_{task['name']}_duet_parallel_heads.pt"
            save_parallel_detect_heads_checkpoint(
                merged_ckpt,
                parallel_head_ckpt,
                old_state=old_state,
                new_state=new_state,
        ...
        history.append(
            {
                ...
                "parallel_head_checkpoint": str(parallel_head_ckpt) if parallel_head_ckpt is not None else None,
