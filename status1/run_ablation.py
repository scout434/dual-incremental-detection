...
BASE_T1_CHECKPOINT = "status1/output/main/task_1_voc_1_10_best.pt"
...
def build_train_config(name: str) -> tuple[dict[str, Any], Path]:
    ...
    output_dir = f"status1/output/ablations/{name}"
...
def build_eval_plan(name: str, train_cfg: dict[str, Any]) -> tuple[dict[str, Any], Path]:
    ...
    plan = {
        "manifest": f"{output_dir}/eval_manifest.json",
        "output": f"{output_dir}/metrics.json",
        "device": 0,
        "metric": "map50",
        "split": "val",
        "clamp_to_one": False,
        "checkpoint_aliases": {
            ...
            "ref_voc_11_20": "status1/output/ref_voc/task_1_voc_11_20_best.pt",
            "ref_clipart_1_10": "status1/output/ref_clipart/task_1_clipart_1_10_best.pt",
        },
        "retention": [
            {
                "name": "RI_VOC_1_10",
        ...
        "generalization": [
            {
                ...
                "denominator": {
                    "checkpoint": "ref_voc_11_20",
                    "data": "data/pascal_voc_yolo/voc_11_20/data.yaml",
                    "prepare_global_labels": False,
                },
            },
            {
                "name": "GI_Clipart_1_10",
                "numerator": {
                    "checkpoint": "final",
                    "data": "data/clipart_paper_yolo/clipart_1_10/data.yaml",
                },
                "denominator": {
                    "checkpoint": "ref_clipart_1_10",
                    "data": "data/clipart_paper_yolo/clipart_1_10/data.yaml",
                    "prepare_global_labels": False,
                },
            },
    ...
    eval_plan_path = SCRIPT_DIR / "configs" / "ablations" / f"{name}_eval.yaml"
...
def materialize(name: str) -> tuple[Path, Path]:
    ...
    index_path = SCRIPT_DIR / "configs" / "ablations" / "index.json"
    index_payload = {
        key: {
            "title": value["title"],
            "train_config": f"status1/configs/ablations/{key}.yaml",
            "eval_plan": f"status1/configs/ablations/{key}_eval.yaml",
            "output_dir": f"status1/output/ablations/{key}",
            "switches": {
                "seq_ft": value["seq_ft"],
...
def main() -> None:
    ...
    for name in selected:
        ...
        print(f"[ablation] train config: {train_cfg_path}")
        print(f"[ablation] eval plan:    {eval_plan_path}")
